...
def compare_specifications(trackvia_df, directus_df, field_mapper):
    ...
    for _, row in trackvia_df.iterrows():
        sku = str(row[trackvia_column]).strip()
        if not sku:
            continue
        trackvia_lookup[sku] = row

    mismatches = []
    directus_skus = directus_df[directus_column].astype(str).str.strip().tolist()
    for sku, row in trackvia_lookup.items():
        if sku not in directus_skus:
            continue

        directus_row = directus_df.loc[directus_df[directus_column].astype(str).str.strip() == sku].iloc[0]
